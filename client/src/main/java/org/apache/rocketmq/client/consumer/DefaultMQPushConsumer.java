...
public class DefaultMQPushConsumer extends ClientConfig implements MQPushConsumer {
     ...
     * </ul>
     *
     * 根据消息进度从消息服务器拉取不到时重新计算消费策略
     * CONSUMER_FROM_LAST_OFFSET: 从队列当前最大偏移量开始消费
     * CONSUMER_FROM_FIRST_OFFSET: 从队列当前最小偏移量开始消费
     * CONSUMER_FROM_TIMESTAMP: 从消费者启动时间戳开始消费
     * 注意：如果从消费进度服务OffsetStore读取到MessageQueue中的偏移量不小于0，
     ...
     * 推模式下拉取任务的间隔时间，默认一次拉取完成继续拉取
     */
    private long pullInterval = 0;

    /**
     * Batch consumption size
     * 消息并发消费时一次消费消息条数，通俗点来说就是每次传入MessageListener#consumerMessage中的消息条数
     */
     ...
     * queue waiting.
     * 最大消费重试次数。如果消息消费次数超过maxReconsumeTimes还未成功，则将消息转移到一个失败队列，等待被删除。
     */
