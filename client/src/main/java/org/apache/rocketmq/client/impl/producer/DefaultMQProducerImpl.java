...
public class DefaultMQProducerImpl implements MQProducerInner {
     ...
     * 2. 创建MQClientInstance实例。整个JVM实列中只存在一个MQClientManager实例
     *    维护一个MQClientInstance缓存表factoryTable
     *    这就意味着同一个clientId只会创建一个MQClientInstance实例
     * 3. 注册MQClientInstance
     *    将当前生产者加入到MQClientInstance管理中，方便后续调用网络请求，进行心跳检查等
     * 4. 启动MQClientInstance。
     *    如果MQClientInstance已经启动，则本次不会真正执行。
     * @param startFactory
     ...
     * 2. 为消息分配全局唯一id
     *    如果消息体超过4k，会对消息体采用zip压缩，并设置消息的系统标记为MessageSysFlag.COMPRESSED_FLAG
    ...
    private SendResult sendKernelImpl(final Message msg,
        ...
        if (brokerAddr != null) {
            ...
            try {
                ...
                if (this.hasSendMessageHook()) { // 是否注册了发送消息的钩子函数
    ...
    public TransactionSendResult sendMessageInTransaction(final Message msg,
        ...
        switch (sendResult.getSendStatus()) {
            case SEND_OK: {
                try {
                    ...
                    // 记录事务消息的本地事务状态
                    // eg. 可以通过将消息唯一id存储在数据中，并且该方法与业务代码处于一个事务，业务事务要么一起成功，要么一起失败
        ...
        try {
            // 结束事务，根据第二步返回的事务状态进行提交、回滚或暂时不处理
    ...
    public void endTransaction(
        ...
        // 根据消息所属的消息队列获取broker的ip与端口信息，然后发送结束命令
